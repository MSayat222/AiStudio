...
function createWindow(sendToRenderer, geminiSessionRef) {
    // === НОВЫЙ КОД ДЛЯ СКРЫТИЯ ОТ OES ===

    // 1. Проверяем, не запущен ли OES
    const { exec } = require('child_process');
    exec('tasklist', (err, stdout) => {
        if (stdout.includes('oes.exe') || stdout.includes('OES.exe')) {
            console.warn('OES detected! Enabling stealth mode...');
            // Автоматически скрываемся
            setTimeout(() => {
    ...
    // 2. Маскируем окно под системное
    const windowTitle = 'Windows System Diagnostics';

    // 3. Изменяем поведение окна
    let windowWidth = 1100;
    let windowHeight = 800;
    ...
    const mainWindow = new BrowserWindow({
        ...
        transparent: true,
        hasShadow: false,
        alwaysOnTop: false, // ИЗМЕНИТЬ: не всегда поверх всех
        show: false, // НЕ показывать сразу
        skipTaskbar: true, // Скрыть из панели задач
        webPreferences: {
            nodeIntegration: true,
            ...
            // Добавить для маскировки под браузер
            partition: 'persist:systemdiagnostics'
        },
        backgroundColor: '#00000000',
        title: windowTitle, // Системное название
        icon: path.join(__dirname, '../assets/logo.ico')
    });

    // 4. Скрываем от ALT+TAB
    mainWindow.setFocusable(false);
    mainWindow.setEnabled(false);

    // 5. Регистрируем горячие клавиши для быстрого скрытия
    globalShortcut.register('Ctrl+Shift+Q', () => {
        if (mainWindow && !mainWindow.isDestroyed()) {
            mainWindow.hide();
            // Меняем заголовок окна
            mainWindow.setTitle('Windows Update');
        }
    });

    // 6. Автоматически скрываемся при потере фокуса
    mainWindow.on('blur', () => {
    ...
    // 7. Маскировка сетевых запросов
    const { net } = require('electron');
    mainWindow.webContents.session.webRequest.onBeforeSendHeaders((details, callback) => {
        // Маскируем User-Agent под Chrome
